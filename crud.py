...
def get_all_authors(
        ...
        skip: int = 0,
        limit: int = 10
) -> list[schemas.AuthorBase]:
    return db.query(models.Author).offset(skip).limit(limit).all()


def get_author(db: Session, author_id: int) -> schemas.AuthorBase:
...
def create_author(
    ...
    author_db = models.Author(name=author.name, bio=author.bio)
    db.add(author_db)
    db.commit()
...
def update_author_model(
        db: Session,
        author: schemas.AuthorBase
) -> schemas.AuthorBase | None:
...
def update_book(
        db: Session,
        book: schemas.BookBase
) -> schemas.BookBase | None:
    book_db = db.query(models.Book).filter(models.Book.id == book.id).first()
    if book_db:
